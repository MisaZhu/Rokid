...
/**
 * Added by Misa.Z
 * All job been done by Kasper, I just rename tree_node_ to TreeNode, and tree to Tree , nothing more !!
 */
...
template<class T>
class TreeNode { // size: 5*4=20 bytes (on 32 bit arch), can be reduced by 8.
	public:
		TreeNode();
		TreeNode(const T&);
...
void Tree<T, tree_node_allocator>::head_initialise_() 
   ...
   head = alloc_.allocate(1,0); // MSVC does not have default second argument 
	...
	alloc_.construct(head, TreeNode<T>());
	alloc_.construct(feet, TreeNode<T>());
...
iter Tree<T, tree_node_allocator>::append_child(iter position)
	...
	alloc_.construct(tmp, TreeNode<T>());
